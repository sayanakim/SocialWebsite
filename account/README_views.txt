...
Мы используем декоратор login_required, так как пользователи 
должны авторизоваться для редактирования своего профиля. В этом 
случае используются две модели форм: UserEditForm для хранения 
данных встроенной модели User и ProfileEditForm для хранения 
дополнительных данных профиля. Чтобы проверить представленные 
данные, мы проверяем, что метод is_valid() обеих форм возвращает 
значение true. В этом случае мы сохраняем обе формы для 
обновления соответствующего объекта в базе данных
